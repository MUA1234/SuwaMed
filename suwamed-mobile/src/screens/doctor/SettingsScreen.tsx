...
import { View, Text, SafeAreaView, StyleSheet, ScrollView, TouchableOpacity, Switch, Alert, Modal, TextInput, ActivityIndicator } from 'react-native';
...
import { colors as lightColors, spacing, borderRadius } from '../../config/theme';
...
const SettingsScreen: React.FC = () => {
    ...
    const languageLabels: Record<string, string> = { en: 'English', si: 'Sinhala', ta: 'Tamil' };
    ...
    const handleChangePassword = async () => {
        if (!currentPw || !newPw || !confirmPw) { showToast('error', 'Missing Fields', 'Please fill in all fields.'); return; }
        if (newPw.length < 8) { showToast('error', 'Too Short', 'New password must be at least 8 characters.'); return; }
        ...
        Alert.alert(
            ...
            [
                { text: t('common.cancel'), style: 'cancel' },
                {
                    ...
                    style: 'destructive',
                    onPress: () => {
                        setDeletePw(''); setDeletePwShow(false); setDeleteModal(true);
                    },
                },
            ],
        );
    ...
    const handleConfirmDelete = async () => {
        if (!deletePw) {
            showToast('error', t('common.error'), t('validation.required')); return;
        }
        ...
        try {
            ...
            showToast('error', t('common.error'), err?.response?.data?.message || t('common.deleteAccountError'));
        } finally {
            ...
            <Modal visible={deleteModal} transparent animationType="slide" onRequestClose={() => setDeleteModal(false)}>
                <View style={s.modalOverlay}>
                    <View style={s.modalCard}>
                        ...
                        <View style={s.pwField}>
                            ...
                            <View style={s.pwInputWrap}>
                                ...
                                <TouchableOpacity onPress={() => setDeletePwShow(!deletePwShow)}>
                                    <MaterialCommunityIcons name={deletePwShow ? 'eye-off' : 'eye'} size={20} color={colors.textSecondary} />
                                </TouchableOpacity>
                            </View>
                        </View>
                        <TouchableOpacity
                        ...
                        >
                            {deleteLoading ? <ActivityIndicator color="#fff" /> : <Text style={s.pwBtnText}>{t('common.deleteAccountConfirmCta')}</Text>}
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
...
const getDynamicStyles = (colors: any) => StyleSheet.create({
    ...
    title: { flex: 1, fontSize: 20, fontWeight: '600', color: colors.textPrimary, textAlign: 'center' },
    scroll: { paddingHorizontal: spacing.xl, paddingBottom: 100 },
    sectionLabel: { fontSize: 12, fontWeight: '600', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: spacing.sm, marginTop: spacing.sm },
    ...
    pwLabel: { fontSize: 12, fontWeight: '600', color: colors.textSecondary, marginBottom: spacing.xs },
    ...
    pwInput: { flex: 1, paddingVertical: spacing.md, fontSize: 16, color: colors.textPrimary },
