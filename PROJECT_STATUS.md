...
| 17 | Phase 3 §3.1.A — Socket.io chat (server setup + Message persistence + ChatScreen) | ✅ 2026-05-14 | **Backend**: new `src/socket/index.ts` instantiates `new Server(httpServer, { cors, pingTimeout, pingInterval })` mirroring the REST CORS allowlist. `initSocket(server)` is idempotent. JWT auth middleware reads `handshake.auth.token` (matches existing `useSocket.ts`) → falls back to `Authorization` header. Authorization predicate `userBelongsToAppointment(appointmentId, user)`: patients must own `appointment.patientId`, doctors must own the matching `Doctor` profile (looked up once via `Doctor.findOne({userId})`); admins bypass. Events: `join_consultation` (joins `consultation:{appointmentId}` room, ack with `{ok, error?}`), `leave_consultation`, `send_message` (validates type ∈ text/image/file, length 1–4000, persists Message doc with `senderId`, `receiverId` resolved via counterparty lookup, then `emit('new_message')` to the consultation room **and** the recipient's personal `user:{id}` room so the inbox updates even when they're off-screen — plus fire-and-forget push notification via `sendToUser` from row 15), `typing_start`/`typing_stop` (emits `user_typing` to others in the room, not the sender), `mark_read` (sets `isRead`/`readAt` atomically and emits `message_read` back to the room). Every user joins their own `user:{id}` personal room on connect for inbox fan-out. `server.ts` calls `initSocket(server)` after `connectDB()`. `chat.routes.ts` rewritten — replaced both 501 stubs with real endpoints: `GET /chat/:appointmentId/messages` (cursor-paginated via `before` ISO timestamp, max 100, sorted oldest→newest after fetch so the client appends without re-sorting) and `GET /chat/:appointmentId` (full transcript capped at 200). Both use the same `userBelongsToAppointment` guard as the socket layer to keep authorization in lockstep. **Mobile**: new `src/screens/shared/ChatScreen.tsx` — connects to `io(SOCKET_URL, { auth: { token: accessToken } })` once on mount, joins the consultation room, fetches `GET /chat/:appointmentId` for history, listens for `new_message`/`user_typing`/`message_read`, auto-acks inbound messages with `mark_read`, debounced typing indicator (1.5s after last keystroke), KeyboardAvoidingView composer, message bubbles with timestamp + read receipt (single check for sent, double-check + blue for read), auto-scroll on new message, empty state, dedupes the optimistic message against the server replay. Uses theme + locales. Disconnects on unmount + emits `leave_consultation`. Registered in `PatientNavigator.AppointmentsStack`, `DoctorNavigator.DashboardStack` and `DoctorNavigator.ScheduleStack`. Patient `AppointmentDetailScreen.handleJoinConsultation` now navigates to `ChatScreen` instead of showing the "Coming Soon"-style Alert; doctor `AppointmentDetailScreen.handleStartConsultation` now navigates to chat right after marking the appointment in-progress, and a separate "Open Chat" button appears while `status === 'in_progress'`. **Locales**: added `common.connecting`, `common.typing`, `common.chat`, `common.openChat`, `common.typeAMessage`, `common.noMessagesYet`, `common.startConversation` to en/si/ta. Backend `npx tsc --noEmit` clean. **Note**: image/file attachments are accepted by the socket layer and round-tripped via the existing `Message` shape, but the mobile composer only sends text right now — the attachment button is a follow-up. The existing `useSocket.ts` hook is now redundant with `ChatScreen`'s direct `io()` connection; left in place because it isn't called from anywhere that matters yet, but a future polish pass can delete it. |
| 18 | Phase 3 §3.1.B — Agora video calls (token endpoint + react-native-agora + VideoCallScreen) | ✅ 2026-05-14 | **Backend**: new `services/agora.service.ts` lazy-loads the `agora-token` package via `require()` so the backend still boots if `npm install` is skipped. `mintAgoraToken({ channelName, uid, role, expiresInSec })` returns `null` when `AGORA_APP_ID`/`AGORA_APP_CERTIFICATE` are missing, when the package can't load, or when token construction throws — every failure path is logged but never thrown. New route `POST /api/consultations/:appointmentId/token` (mounted in existing `consultation.routes.ts`): looks up the appointment, refuses non-video types, refuses non-active statuses (only `confirmed` / `in_progress`), enforces the same patient/doctor ownership predicate as the chat layer, derives a 32-bit `uid` from the trailing 8 hex chars of the User `_id` (modulo 2_000_000_000) so the two parties get distinct UIDs but the same appointment always gives stable UIDs, computes `channelName = appt_{appointmentId}`, mints a 1-hour RTC publisher token, persists `consultation.agoraChannelName` on first mint, returns `{ appId, channelName, token, uid, expiresInSec }`. `agora-token@^2.0.5` added to backend `package.json`. Backend `npx tsc --noEmit` clean. **Mobile**: `react-native-agora@^4.5.1` added to `package.json`. New `screens/shared/VideoCallScreen.tsx`: dynamic `require('react-native-agora')` wrapped in try/catch so the screen mounts in Expo Go and renders an instructional error card ("Run `eas build --profile development --platform android` to enable video") instead of crashing the bundle. In a dev-client / production build the require succeeds and the full call runs: POSTs to `/consultations/:id/token`, calls `createAgoraRtcEngine().initialize({ appId, channelProfile: Communication })`, `setClientRole(Broadcaster)`, `enableVideo()` / `enableAudio()` / `startPreview()`, `joinChannel(token, channelName, uid, …)` with `publishCameraTrack`/`publishMicrophoneTrack` + auto-subscribe. Event handlers track `onJoinChannelSuccess`, `onUserJoined` (sets `remoteUid`), `onUserOffline`, `onError`. UI is a full-screen `RtcSurfaceView` for the remote video, a small picture-in-picture for the local preview (hidden when camera is off), and a controls bar with 5 round buttons: mute mic (`muteLocalAudioStream`), camera off (`muteLocalVideoStream`), flip camera (`switchCamera`), toggle speaker (`setEnableSpeakerphone`), and end call (red, `leaveChannel` + `release` + `goBack`). Cleanup on unmount calls `leaveChannel()` + `release()`. Registered in `PatientNavigator.AppointmentsStack`, `DoctorNavigator.DashboardStack` and `DoctorNavigator.ScheduleStack` with `gestureEnabled: false` so the swipe-back doesn't accidentally drop the call. **Patient `AppointmentDetailScreen`**: `handleJoinConsultation` now branches by `appointment.type` — video → `VideoCallScreen`, anything else → `ChatScreen`. For video appointments an additional "Open Chat" secondary button appears alongside the join button so patients can chat while waiting. **Doctor `AppointmentDetailScreen`**: `handleStartConsultation` navigates to `VideoCallScreen` (for video) or `ChatScreen` (otherwise) right after marking the appointment in-progress. While `status === 'in_progress'` an explicit "Join Video Call" button is shown for video appointments, plus the existing "Open Chat" button. **Blocked-on-user — this is the one-way door called out in §3.1.B**: `react-native-agora` is **not Expo Go compatible**. Once installed, the project must always be run as an EAS dev-client / production build — `expo start` against Expo Go will crash on native module resolution. Required to test: (1) `cd suwamed-mobile && npm install` to pull `react-native-agora`; (2) `eas build --profile development --platform android` (this is the moment of no return — every dev from here on uses the dev client); (3) verified `AGORA_APP_ID` + `AGORA_APP_CERTIFICATE` env vars on the deployed backend; (4) test on a physical Android device (the emulator's camera/mic is unreliable for SDK-quality testing). The screen currently renders the instructional fallback inside Expo Go, so existing devs aren't forced to migrate until they're ready. |
...
| 20 | Phase 4 — Half-built features (subscriptions, reschedule, notes, support tickets, system logs, reviews CRUD, GPS pharmacies, scheduled medication reminders, pagination) | ⏳ 2026-05-08 | **Implemented:** (a) `PUT /appointments/:id/reschedule` — patient/doctor authorization, status reset to pending, audit trail appended to `notes`; mobile `appointment.api.ts` already calls it. (b) `PUT /appointments/:id/notes` — doctor/admin only, 5000-char cap. (c) Reviews CRUD — `GET /reviews/:id`, `PUT /reviews/:id` (24-hour edit window, recomputes doctor rating), `DELETE /reviews/:id` (own or admin, recomputes rating, unsets appointment.review). Single `recomputeDoctorRating` helper avoids drift. (d) `PATCH /health-tips/:id/view` (public) — increments `viewCount`; mobile `HealthTipDetailScreen` calls it once on mount via a `useRef` guard. (e) Notification badge unread count — bound `useNotificationStore.unreadCount` to bell icon on patient `HomeScreen` and doctor `DashboardScreen`; both fetch `/notifications` alongside their primary fetches and render a numbered badge (or hidden when 0); old static red dot CSS retained but unused. (f) Admin pending-doctors pagination + search — `getPendingDoctors` now accepts `page`/`limit`/`search` (matches against SLMC number and specialization), defaults to page 1 / 20 docs (backwards-compatible with current mobile callers). (g) Doctor blocked-slot UI — backend `GET/POST/DELETE /doctors/blocked-slots[/:index]` with HH:MM and date validation; mobile `SetAvailabilityScreen` gained a Blocked Time Off section with list + add form (date / start / end / reason). (h) GPS pharmacies map — `NearbyPharmaciesScreen` "Directions" button now opens Google Maps via `Linking.openURL("https://www.google.com/maps/search/?api=1&query=...")`; phone numbers tappable via `tel:`. **Coming Soon kill** done in same pass: `admin/AdminSettingsScreen` now ships an inline change-password modal (replaces "Account Security coming soon" alert); `admin/ManagementScreen` removed Data Export / Backup & Restore / Support Tickets cards (kept Content Management, Health Tips, System Settings); `admin/MoreScreen` replaced System Logs / Help & Documentation entries with Privacy Policy + Terms + a Contact Support `mailto:` action; `admin/SystemSettingsScreen` Maintenance Mode now shows an explanatory dialog (controlled via env var on the host) instead of a "coming soon" stub; `doctor/AppointmentDetailScreen` "Coming Soon" alert on start consultation rewritten to a clean "in-progress" notification; `patient/AppointmentDetailScreen` join-consultation alert rewritten with a real description; `doctor/DoctorProfileScreen` Language row now opens an Alert.alert language picker that drives `useSettingsStore.setLanguage` (en/si/ta), mirrors `AdminSettingsScreen`; `patient/SubscriptionScreen` upgrade alert rewritten to a `mailto:support@suwamed.lk` flow with pre-filled body. **Verification:** `npx tsc --noEmit` clean for backend (only previous icon-name warnings in admin/SystemSettingsScreen.tsx and patient/NearbyPharmaciesScreen.tsx remain — both pre-existing per §3.4 and unrelated to these changes). All three locale JSONs still parse. **Still open under this row:** subscription CRUD against the `Subscription` model, support tickets controller+UI, system logs middleware, scheduled medication reminders via `expo-notifications`, system-wide pagination on long lists. **2026-05-14 follow-up — all but pagination now landed:** (i) `controllers/subscription.controller.ts` exposes `GET /subscriptions/plans`, `GET /subscriptions`, `POST /subscriptions` (validates plan ∈ free/basic/premium, computes endDate from `SubscriptionPlan.duration`, writes to `Patient.subscription`, fires `subscription_renewed` push), `DELETE /subscriptions` (turns off auto-renew). Plans come from the `SubscriptionPlan` collection when populated, else from a deterministic in-memory fallback (`FALLBACK_PLANS`). (ii) `controllers/supportTicket.controller.ts` + `routes/supportTicket.routes.ts` mounted at `/api/support-tickets`: `POST /` (create with category/priority validation), `GET /` (paginated; admin sees all, users see own), `GET /:id`, `POST /:id/messages` (owner or admin reply, admin reply flips status `open→in_progress` and pushes `support_reply` to owner), `PATCH /:id/status` (admin only). Mobile-side: new `screens/patient/SubmitTicketScreen.tsx` with category chips + subject/description fields + POSTs to `/support-tickets`; navigated from a new "Submit a support ticket" CTA on `HelpSupportScreen`. (iii) `services/systemLog.service.ts` exports `logEvent(action, { actorId, targetId, details, req })` — writes to `SystemLog`, captures IP + user-agent, failures swallowed. Wired into `admin.controller.ts` `verifyDoctor` / `rejectDoctor` / `updateUserStatus`. New `GET /api/admin/system-logs` paginated viewer endpoint. (iv) `screens/patient/MedicationRemindersScreen.tsx` now actually schedules local notifications: `scheduleReminderNotifications(reminder)` parses HH:MM, requests permission, fans out one `Notifications.scheduleNotificationAsync` per selected day using a `weekday/hour/minute repeats: true` CalendarTriggerInput against the `default` Android channel, returns the array of notification IDs. `cancelReminderNotifications(reminder)` calls `cancelScheduledNotificationAsync` for each. Wired into add (schedules immediately), delete (cancels first), and toggle (cancels when disabling, reschedules when enabling); IDs persisted in the same AsyncStorage payload so app restarts don't orphan them. (v) `HelpSupportScreen` email/phone Alert stubs replaced with real `Linking.openURL('mailto:…')` / `tel:…` calls. **Still deferred:** system-wide pagination on the *patient/doctor* list screens (admin user pagination is already paged; patient-side infinite scroll on doctor search + my-appointments is a follow-up — backend already supports `page`/`limit`). Mobile-side admin viewer screens for the new system-logs + support-tickets endpoints are not built — they're addressable via direct API for now (the admin Management screen lost its Support Tickets tile in the previous pass per this row). |
| 21 | Phase 5 — Polish (skeletons, dark-mode QA, accessibility, Swagger, tests) | ⏳ 2026-05-14 (partial) | **Swagger ✅**: `src/config/swagger.ts` builds a hand-curated OpenAPI 3.0 spec covering auth, appointments, consultations (Agora token), chat history, symptoms (with the new `source` field documented), notifications + push token, subscriptions, support tickets, and the admin system-logs viewer. Mounted at `/api/docs` (interactive Swagger UI) and `/api/openapi.json` (raw JSON) in `server.ts` ahead of the `/api` rate limiter so the docs page isn't throttled. `swagger-jsdoc` + `swagger-ui-express` were already in `package.json`. **Tests ✅ (baseline)**: `jest.config.js` (ts-jest preset, runs `tests/**/*.test.ts`), `tests/setup.ts` (seeds JWT secrets, disables reminder cron, supplies test Firebase creds so admin SDK init doesn't crash), `tests/smoke.test.ts` (4 cases: 404 envelope, `/auth/me` 401 without token, `/openapi.json` returns OpenAPI 3.x doc, `/auth/login` validator rejects empty body). Added `jest`, `ts-jest`, `supertest`, `@types/jest`, `@types/supertest` to devDeps. `npm test` script wired. Backend `npx tsc --noEmit` clean. **Deferred (manual UI work, not delegable to code edits without a device QA pass)**: (a) skeleton states on every list screen — the `<Skeleton>` component exists and is used on a few screens; visiting every list (admin users, patient appointments, doctor schedule, prescriptions, health records, reviews, health tips, symptom history, notifications, …) to swap `ActivityIndicator` for `Skeleton` is ~3 hours of mechanical work; (b) dark-mode visual QA — every screen consumes the `useTheme()` hook so colors come from the theme, but contrast + spacing has to be eyeballed on a real device in both modes; (c) accessibility labels — `accessibilityLabel` on tappables and dynamic-font-size checks; (d) mobile e2e via Detox/Maestro. These are the §3.4 polish items and remain open. **Note**: Swagger paths are hand-curated rather than generated from `@openapi` JSDoc — adding route-level annotations is a future improvement; the current spec is enough for QA and integration partners to navigate the API. |
